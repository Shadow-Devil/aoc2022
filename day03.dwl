...
var parsed = lines(payload) map toChars($)

fun solve(first: Array<String>, others: Array<Array<String>>): Number = 
...
---
{
    part1: parsed map (halve($) then solve($.l, [$.r])) then sum($),
    part2: parsed divideBy 3 map solve($[0], $ drop 1) then sum($)
}